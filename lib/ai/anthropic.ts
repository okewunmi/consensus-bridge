...
${summary}`
  ...
  const message = await anthropic.messages.create({
    ...
    system: 'You are a neutral political analyst helping facilitate cross-partisan dialogue. Be balanced, insightful, and constructive. Return ONLY valid JSON.'
  })
...
Respond in ~100 words. Acknowledge perspectives genuinely, introduce contrasting views respectfully, find shared values, and ask a deepening question.`
  ...
  const message = await anthropic.messages.create({
    ...
    system: 'You are a neutral democratic dialogue facilitator. Present all perspectives with equal respect. Never take sides. Help people find shared values beneath disagreements.'
  })
...
This should be an authentic synthesis, NOT a watered-down compromise.`
  ...
  const message = await anthropic.messages.create({
    ...
    system: 'You are a democratic policy synthesis expert. Generate concrete, actionable consensus from dialogue. Be specific and authentic.'
  })
